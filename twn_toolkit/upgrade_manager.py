...
def _safe_relative_path(value: str) -> str:
    ...
    if normalized.startswith(".") and not (
        normalized in {RELEASE_MANIFEST, ".gitignore"} or normalized.startswith(".github/")
    ):
    ...
    *,
    ...
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            if manifest.get("format") != UPGRADE_FORMAT or manifest.get("product") != PRODUCT_ID:
            ...
            if current_version:
                ...
                if current < parse_version(minimum):
                    raise UpgradeError(
                        f"This bundle requires v{minimum} or newer; this installation is v{current_version}."
                    )
            ...
            expected_names = {f"payload/{_safe_relative_path(str(name))}" for name in files}
            actual_names = {name for name in names if name != "manifest.json" and not name.endswith("/")}
