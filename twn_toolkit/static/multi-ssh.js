(() => {
  ...
  const gridPanel = form.querySelector("[data-ssh-matrix-grid]");
  const rawPanel = form.querySelector("[data-ssh-matrix-raw]");
  ...
  const parseMatrix = (source) => {
    ...
    if (!delimiter) throw new Error("Separate matrix columns with pipes, tabs, or commas.");
    const parsed = lines.map((line) => parseLine(line, delimiter));
    const sourceHeaders = parsed[0];
    if (sourceHeaders.length > 20) throw new Error("A maximum of 20 matrix columns is allowed.");
  ...
  const syncRawMatrix = () => {
    ...
    matrix.value = [
      ...
      ...populatedRows.map((row) => row.map(escapeMatrixValue).join(" | ")),
    ].join("\n");
  };
  ...
  const validateGrid = ({ focus = false, requireTargets = false } = {}) => {
    ...
    const headerInputs = [...matrixHead.querySelectorAll("[data-ssh-header-index]")];
    ...
    if (requireTargets && !populatedCount && !firstInvalid) {
      message = "Add at least one target before previewing.";
    }
  ...
  const headerVariables = () => {
    if (matrixMode === "grid") return headers.map((header) => normalize(header.label));
    try {
      return parseMatrix(matrix.value).headers.map((header) => normalize(header.label));
    } catch (_error) {
      return [];
    }
  };
  ...
  const updateVariablePicker = () => {
    ...
    [...new Set(headerVariables())]
      .filter((name) => /^[a-z][a-z0-9_]*$/.test(name) && !builtIns.has(name))
      .forEach((name) => {
  ...
  matrixHead.addEventListener("click", (event) => {
    ...
    rows = rows.map((row) => row.filter((_value, columnIndex) => columnIndex !== index));
    renderGrid();
    matrixChanged();
  });
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (matrixMode === "grid") syncRawMatrix();
    ...
    const saveWithMatrix = action === "save_commandlet"
      ...
      && matrix.value.trim();
    if (matrixMode === "grid" && (matrixRequired || saveWithMatrix)
      && !validateGrid({ focus: true, requireTargets: matrixRequired })) {
      event.preventDefault();
      return;
    }
    if (matrixMode === "raw" && (matrixRequired || saveWithMatrix)) {
      try {
        parseMatrix(matrix.value);
        setMatrixError();
      } catch (error) {
        event.preventDefault();
        setMatrixError(error.message);
