(() => {
  ...
  document.querySelectorAll("form.automation-form").forEach((form) => {
    ...
    const snmpBuilder = form.querySelector("[data-snmp-rule-builder]");
    if (snmpBuilder) {
      const hidden = form.querySelector("[data-snmp-rules-json]");
      const list = snmpBuilder.querySelector("[data-snmp-rule-list]");
      const empty = snmpBuilder.querySelector("[data-snmp-rule-empty]");
      const add = snmpBuilder.querySelector("[data-add-snmp-rule]");
      let choices = [];
      let rules = [];
      try { choices = JSON.parse(snmpBuilder.dataset.oidChoices || "[]"); } catch (_error) { choices = []; }
      try { rules = JSON.parse(hidden?.value || "[]"); } catch (_error) { rules = []; }
      ...
      const sync = () => { if (hidden) hidden.value = JSON.stringify(rules); };
      const render = () => {
        list.replaceChildren();
        empty.hidden = rules.length > 0;
        rules.forEach((rule, index) => {
