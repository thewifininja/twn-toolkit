...
and compatibility/fallback ping. Accelerated fping and SNMP retain their separate
execution paths and do not consume these slots.
...
that concurrency. Default TCP condition concurrency therefore changes from up
to 100 per condition to up to 20 across these condition batches. Large checks may
