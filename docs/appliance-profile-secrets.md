# Saved appliance profile secrets

FortiGate API keys and FortiAuthenticator passwords are now encrypted in their existing profile JSON files. Public profile fields remain readable. Store callers still receive strings, so device requests, profile edits, duplication, previews, and configuration backup workflows keep their existing interfaces.
...
Restart all readers/writers on the updated version afterward. The command handles the two existing appliance profile files; absent files are not created. It is repeatable and reports filenames, never secret values. Each file is replaced atomically, but the two-file migration is not a single transaction. A failure may leave one file migrated and the other unchanged; correct the error and rerun.
...
RADIUS/SNMP credentials, automation secrets, other integrations, and RADIUS EAP subprocess arguments remain separate audit work. No live migration, password rotation, appliance call, or enrollment change is performed by merely shipping this implementation.
